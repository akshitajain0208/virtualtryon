...
def predict():
    ...
    while True:
        ...
        for (x,y,w,h) in faces:  #strt pt  end pt
           # cv2.rectangle(img,(x,y),(x+w,y+h),(255,0,0),2)
            #cv2.rectangle(img,(x-100,y+100),(x+w-100,y+h+100),(255,255,255),2)
        ...
        # take ROI for pant from background equal to size of pant image
            ...
            # Blur the original input  image/feed when filter is superimposed 
            
            #slecting the area to blurr
    ...
    #|||||||||||||||||||||||||||||||SHIRT||||||||||||||||||||||||||||||||||||||||
            ...
            # take ROI for shirt from video equal to size of shirt image
            ...
            break
        cv2.putText(img,"Shirt Width: {}".format(shirtWidth),(50,50),cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,255),2,cv2.LINE_4)
        cv2.putText(img,"Shirt Height: {}".format(shirtHeight),(50,80),cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,255),2,cv2.LINE_4)
        cv2.putText(img,"Pant Width: {}".format(pantWidth,pantHeight),(50,110),cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,255),2,cv2.LINE_4)
    ...
    return render_template('index.html', size_detected=random.choice(['L','XL']))
